...
$insert_cart = "INSERT INTO tbldondat(ThoiGian,IdTKNV,IdTKKH,TrangThai,TongTien) VALUE('" . $now . "',4,'" . $id_khachhang . "',1,'" . $tongtien . "')";
...
if ($cart_query) {
	...
	foreach ($_SESSION['cart'] as $key => $value) {
		...
		$insert_order_details = "INSERT INTO tblchitietdd(IdSP,IdDD,SoLuongMua) VALUE('" . $id_sanpham . "','" . $IdDD . "','" . $soluong . "')";
		...
		$sql_update_slsp = "UPDATE tblsanpham SET SoLuong = '$soluongNew' WHERE IdSP = '$id_sanpham'";
		mysqli_query($mysqli, $sql_update_slsp);
	}

	$sql_lietke_kh = "SELECT * FROM tbldondat,tbltaikhoan WHERE tbldondat.IdTKKH=tbltaikhoan.IdTK AND tbldondat.IdDD = '" . $IdDD . "' ORDER BY tbldondat.IdDD DESC";
