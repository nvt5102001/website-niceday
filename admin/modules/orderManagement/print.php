<?php
	...
	$pdf->Write(10,"Mã hoá đơn: ".$row_data['ReInvoiceID']."");
	$pdf->Ln(10);
	$pdf->Write(10,"Ngày đặt: ".$row_data['TimeOrder']."");
	$pdf->Ln(20);

	$pdf->Write(10,"Tên khách hàng: ".$row_data['AccountName']."");
	$pdf->Ln(10);
	...
	while($row = mysqli_fetch_array($query_lietke_dh)){
		$i++;
		$tongtien += ($row['QuantityReOrder']*$row['PriceReOrder']);
	$pdf->Cell($width_cell[0],10,$i,1,0,'C',$fill);
	$pdf->Cell($width_cell[1],10,$row['ReInvoiceID'],1,0,'C',$fill);
	$pdf->Cell($width_cell[2],10,$row['ProductName'],1,0,'C',$fill);
	$pdf->Cell($width_cell[3],10,$row['QuantityReOrder'],1,0,'C',$fill);
	$pdf->Cell($width_cell[4],10,number_format($row['PriceReOrder']),1,0,'C',$fill);
	$pdf->Cell($width_cell[5],10,number_format($row['QuantityReOrder']*$row['PriceReOrder']),1,1,'C',$fill);
	$fill = !$fill;
