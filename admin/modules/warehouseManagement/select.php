<?php
$sql_lietke_pnk = "SELECT * FROM tblimport_invoice,tblaccount WHERE tblimport_invoice.SupplierAccID = tblaccount.AccountID ORDER BY tblimport_invoice.ImInvoiceID DESC";
$query_lietke_pnk = mysqli_query($mysqli, $sql_lietke_pnk);

$sql_Status = "SELECT * FROM tblimport_invoice ORDER BY ImDetailID DESC";
              ...
              <table class="table table-dark">
                ...
                <tbody>
                  ...
                  ?>
                    <tr>
                      <td>
                        <a href="modules/warehouseManagement/handle.php?ImInvoiceID=<?php echo $row['ImInvoiceID'] ?>" onClick="return confirm('Bạn có muốn xoá hoá đơn này không?')">
                          <i class="mdi mdi-delete menu-icon" title="Xoá" style="font-size:20px;color:#fff;"></i>
                        </a>
                      </td>
                      <td><?php echo $row['ImInvoiceID'] ?></td>
                      <td><?php echo $row['AccountName'] ?></td>
                      <td><?php echo $row['DeliverName'] ?></td>
