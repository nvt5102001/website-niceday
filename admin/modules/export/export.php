<?php 
    ...
    $sql = "SELECT * FROM tblstatistical" ;
	...
	$objPHPExcel->getActiveSheet()->setCellValue('B2', 'Ngày đặt');
	...
	while ($row = mysqli_fetch_assoc($sql_query)) {
        ...
        $ngaydat = date('d/m/Y', strtotime($row['Time']));
        $objPHPExcel->getActiveSheet()->setCellValue('B' . $i, $ngaydat)
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('C' . $i, $row['TotalInvoice'] )
        ->getStyle('C' . $i)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
        $objPHPExcel->getActiveSheet()->setCellValue('D' . $i, $row['Revenue'])
        ->getStyle('D' . $i)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
        $objPHPExcel->getActiveSheet()->setCellValue('E' . $i, $row['TotalProduct'])
